...
These elements are described in different folders. All the modelling happens within the `openfisca_france` folder.
...
```sh
git clone git://github.com/Ndpnt/openfisca-france.git.git
cd openfisca-france.git
...
```sh
openfisca serve --port 5000 --country-package openfisca_france
```
...
curl -X POST -H "Content-Type: application/json" \
  -d @./openfisca_france/situation_examples/couple.json \
